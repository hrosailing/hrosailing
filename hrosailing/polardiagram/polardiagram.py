...
def depickling(pkl_path):
    ...
    try:
        ...
        raise FileReadingException(
            f"Can't find/open/read {pkl_path}")
...
def symmetric_polar_diagram(obj):
    ...
    if not isinstance(obj, (PolarDiagramTable,
        ...
        raise PolarDiagramException(
            f"Functionality for Type {type(obj)} "
            f"not yet implemented")
...
class PolarDiagramTable(PolarDiagram):
    ...
    def __init__(self, ws_res=None, wa_res=None,
        ...
        bsps = np.ravel(bsps)
        wind_arr = _convert_wind(np.column_stack(
    ...
    def polar_plot_slice(self, ws, ax=None, **plot_kw):
        ...
        of self.wind_speeds
        """
        logger.info(f"Method 'polar_plot_slice("
                    f"ws={ws}, ax={ax}, "
    ...
    def flat_plot_slice(self, ws, ax=None, **plot_kw):
        ...
        speed resolution.
        """
        logger.info(f"Method 'flat_plot_slice("
                    f"ws={ws}, ax={ax}, "
                    f"plot_kw={plot_kw})' called")
    ...
    def polar_plot(self, ws_range=None, ax=None,
        ...
        """
        logger.info(f"Method 'polar_plot("
                    f"ws_range={ws_range}, "
    ...
    def flat_plot(self, ws_range=None, ax=None,
        ...
        the wind speed resolution
        """
        logger.info(f"Method 'flat_plot("
                    f"ws_range={ws_range}, "
                    f"ax={ax}, colors={colors}, "
                    f"show_legend={show_legend},"
                    f"legend_kw={legend_kw}, "
    ...
    def plot_3d(self, ax=None, colors=('blue', 'blue')):
        ...
        colors : tuple of length 2, optional
            ...
            ('blue', 'blue')
        """
        logger.info(f"Method 'plot_3d(ax={ax}, "
                    f"colors={colors})' called")
...
class PolarDiagramCurve(PolarDiagram):
    ...
    *params : Arguments
    ...
    def polar_plot_slice(self, ws, ax=None,
                         **plot_kw):
        """Creates a polar plot
        of a given slice of
        the polar diagram
    ...
    def flat_plot_slice(self, ws, ax=None,
                        **plot_kw):
        """Creates a cartesian
        plot of a slice of the
        polar diagram
    ...
    def polar_plot(self, ws_range=(0, 20, 5), ax=None,
        ...
        if isinstance(ws_range, tuple):
            ...
            ws_range = list(np.linspace(ws_lower,
                                        ws_upper,
                                        ws_step))
    ...
    def plot_3d(self, ws_range=(0, 20, 100), ax=None,
        ...
        if self.radians:
            bsp, wa = (bsp_arr * np.cos(wa_arr),
                       bsp_arr * np.sin(wa_arr))
        else:
            wa_arr = np.deg2rad(wa_arr)
            bsp, wa = (bsp_arr * np.cos(wa_arr),
                       bsp_arr * np.sin(wa_arr))

        return plot_surface(ws_arr, wa, bsp,
                            ax, colors)

    def plot_color_gradient(self, ws_range=(0, 20, 100),
...
class PolarDiagramPointcloud(PolarDiagram):
    ...
    def __repr__(self):
        return (f"PolarDiagramPointcloud(" 
                f"data={self.points})")
    ...
    def add_points(self, new_pts, tw=True):
        ...
        to a fitting shape
        """
        logger.info(f"Method 'add_points("
                    f"new_pts{new_pts}, "
                    f"tw={tw})' called")
    ...
    def polar_plot(self, ws_range=(0, np.inf),
                   ax=None, colors=('green', 'red'),
    ...
    def flat_plot(self, ws_range=(0, np.inf),
                  ax=None, colors=('green', 'red'),
                  show_legend=True, legend_kw=None,
                  **plot_kw):
        """Creates a cartesian
    ...
    def plot_3d(self, ax=None, **plot_kw):
        ...
        ws, wa, bsp = (self.points[:, 0],
                       ...
                       self.points[:, 2])
        wa = np.deg2rad(wa)
        bsp, wa = (bsp * np.cos(wa),
                   bsp * np.sin(wa))
        return plot3d(ws, wa, bsp, ax,
                      **plot_kw)
    ...
    def plot_convex_hull_slice(self, ws, ax=None,
        ...
        the point cloud
        """
        logger.info(f"Method 'plot_convex_hull_slice("
                    f"ws={ws}, ax={ax}, "
                    f"**plot_kw={plot_kw})' called")
