...
class PolarDiagramTableTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.bsps = np.array([[1,   2,   3,   4],
                              [1.5, 2.4, 3.1, 4.1],
                              [1.7, 2.6, 3.5, 4.4],
                              [2,   3,   3.8, 4.6]])
        self.polar_diagram = pol.PolarDiagramTable(self.ws_res,
                                                   self.wa_res,
                                                   self.bsps)

    def test_default_init(self):
        polar_diagram = pol.PolarDiagramTable()
        self.assertEqual(polar_diagram.wind_speeds,
                         np.arange(2, 42, 2))
        self.assertEqual(polar_diagram.wind_angles,
                         np.arange(0, 360, 5))
        self.assertEqual(polar_diagram.boat_speeds,
                         np.zeros((72, 20)))

    def test_init(self):
        self.assertEqual(self.polar_diagram.wind_speeds,
                         self.ws_res)
        self.assertEqual(self.polar_diagram.wind_angles,
                         self.wa_res)
        self.assertEqual(self.polar_diagram.boat_speeds,
                         self.bsps)
    ...
    def test_symmetric_polar_diagram_no_180(self):
        ...
        sym_wa_res = np.concatenate([self.wa_res,
                                     360 - np.flip(self.wa_res)])
        sym_bsps = np.row_stack((self.bsps,
                                 np.flip(self.bsps, axis=0)))

        self.assertEqual(polar_diagram.wind_speeds,
                         self.polar_diagram.wind_speeds)
        self.assertEqual(polar_diagram.wind_angles,
                         sym_wa_res)
        self.assertEqual(polar_diagram.boat_speeds,
                         sym_bsps)
    ...
    def test_symmetric_polar_diagram_w_180_and_0(self):
        wa_res = [0, 90, 180]
        polar_diagram = pol.PolarDiagramTable(ws_res=self.ws_res,
                                              wa_res=wa_res,
                                              bsps=self.bsps[:3, :])
        ...
        self.assertEqual(polar_diagram.wind_angles,
                         sym_wa_res)
        sym_bsps = np.row_stack((self.bsps[:3, :],
                                 self.bsps[1, :]))
        self.assertEqual(polar_diagram.boat_speeds,
                         sym_bsps)
    ...
    def test_change_one_column(self):
        self.polar_diagram.change_entries(new_bsps=[3.5, 3.7, 3.9, 4.1],
                                          ws=8)
    ...
    def test_change_one_row(self):
        self.polar_diagram.change_entries(new_bsps=[1.9, 2.7, 3.6, 4.4],
                                          wa=20)
    ...
    def test_change_subarray(self):
        self.polar_diagram.change_entries(new_bsps=[2.3, 3.0, 2.5, 3.4],
                                          ws=[4, 6], wa=[15, 20])
    ...
    def test_get_one_slice(self):
        self.assertEqual(self.polar_diagram._get_slice_data(2),
                         self.bsps[:, [0]])

    def test_get_multiple_slices(self):
        self.assertEqual(self.polar_diagram._get_slice_data([2, 4, 8]),
                         self.bsps[:, [0, 1, 3]])

    def test_get_all_slices(self):
        self.assertEqual(self.polar_diagram._get_slice_data(None),
                         self.bsps)
...
class PolarDiagramPointCloudTest(unittest.TestCase):
    ...
    def test_init(self):
        self.assertEqual(self.point_cloud.points,
                         self.points)
    ...
    def test_wind_speeds(self):
        self.assertEqual(self.point_cloud.wind_speeds,
                         [2, 4, 6, 8])

    def test_wind_angles(self):
        self.assertEqual(self.point_cloud.wind_angles,
                         [10, 15, 20, 25])
    ...
    def test_add_points_with_no_points(self):
        ...
        self.assertEqual(point_cloud.points,
                         self.points)

    def test_add_points(self):
        self.point_cloud.add_points([[2.3, 15.5, 1.65],
                                     [3.7, 20.1, 2.43]])
        self.points = np.row_stack((self.points,
                                    np.array([[2.3, 15.5, 1.65],
                                              [3.7, 20.1, 2.43]])))
        self.assertEqual(self.point_cloud.points,
                         self.points)
    ...
    def test_symmetric_polar_diagram(self):
        ...
        pts = np.row_stack((self.point_cloud.points,
                            sym_pts))
    ...
    def test_get_slices_list(self):
        ...
        self.assertEqual(wa[0],
                         self.point_cloud._get_slice_data(4)[0])
        self.assertEqual(bsp[0],
                         self.point_cloud._get_slice_data(4)[1])
        self.assertEqual(wa[1],
                         self.point_cloud._get_slice_data(8)[0])
        self.assertEqual(bsp[1],
                         self.point_cloud._get_slice_data(8)[1])
    ...
    def test_get_slices_range(self):
        ...
        self.assertEqual(bsp[0],
                         self.point_cloud._get_slice_data(4)[1])
        self.assertEqual(wa[1],
                         self.point_cloud._get_slice_data(6)[0])
        self.assertEqual(bsp[1],
                         self.point_cloud._get_slice_data(6)[1])
        self.assertEqual(wa[2],
                         self.point_cloud._get_slice_data(8)[0])
        self.assertEqual(bsp[2],
                         self.point_cloud._get_slice_data(8)[1])
...
def point_cloud_suite():
    ...
    suite.addTests([
        ...
        PolarDiagramPointCloudTest('test_add_points_exception_empty_new_pts'),
        PolarDiagramPointCloudTest('test_add_points_exception_wrong_shape'),
        PolarDiagramPointCloudTest('test_symmetric_polar_diagram_no_points'),
...
class PolarDiagramCurveTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.params = (1, 2)
        self.radians = False
        self.polar_curve = pol.PolarDiagramCurve(self.f,
                                                 self.params,
                                                 radians=self.radians)

    def test_init(self):
        self.assertEqual(self.polar_curve.curve.__name__,
                         'test_func')
        self.assertEqual(self.polar_curve.parameters,
                         (1, 2))
        self.assertEqual(self.polar_curve.radians,
                         False)
    ...
    def test_get_wind_angles(self):
        self.addTypeEqualityFunc(np.ndarray, equal_arrays)
        self.assertEqual(self.polar_curve._get_wind_angles(),
                         np.linspace(0, 360, 1000))

    def test_get_wind_angles_radians(self):
        self.addTypeEqualityFunc(np.ndarray, equal_arrays)
        polar_curve = pol.PolarDiagramCurve(self.f, self.params,
                                            radians=True)
        self.assertEqual(polar_curve._get_wind_angles(),
