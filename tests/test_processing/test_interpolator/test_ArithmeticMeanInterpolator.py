...
class TestIDWInterpolator(TestCase):
    def setUp(self) -> None:
        ...
        self.distr = lambda distances, old_weights, *params: np.ones(len(distances))
        self.params = (1,)

        self.wpts = dt.WeightedPoints(
            np.array([[1, 1, 1], [-1, 1, 1], [-1, -1, .5], [1, -1, 1]]),
            np.ones(4))
    ...
    def test_repr(self):
        ...
        result = repr(itp.ArithmeticMeanInterpolator(self.s, self.norm, self.distr, self.params))
        expected_result = f"ArithmeticMeanInterpolator(s={self.s}," \
    ...
    def test_interpolate_default(self):
        ...
        result = itp.ArithmeticMeanInterpolator().interpolate(self.wpts, self.grid_pt)
    ...
    def test_interpolate_custom_s(self):
        ...
        result = itp.ArithmeticMeanInterpolator(self.s).interpolate(self.wpts, self.grid_pt)
    ...
    def test_interpolate_custom_norm(self):
        ...
        result = itp.ArithmeticMeanInterpolator(norm=self.norm).interpolate(self.wpts, self.grid_pt)
        expected_result = 0.875
        self.assertEqual(result, expected_result,
                         f"Expected {expected_result} but got {result}!")
    ...
    def test_interpolate_custom_distribution(self):
        ...
        result = itp.ArithmeticMeanInterpolator(distribution=self.distr).interpolate(self.wpts, self.grid_pt)
        expected_result = 0.875
        self.assertEqual(result, expected_result,
                         f"Expected {expected_result} but got {result}!")
    ...
    def test_interpolate_custom_params(self):
        ...
        result = itp.ArithmeticMeanInterpolator(params=self.params).interpolate(self.wpts, self.grid_pt)
        expected_result = 0.875
        self.assertEqual(result, expected_result,
                         f"Expected {expected_result} but got {result}!")
    ...
    def test_interpolate_edge_grid_pt_in_wpts(self):
        ...
        result = itp.ArithmeticMeanInterpolator().interpolate(
            dt.WeightedPoints(np.array([[-1, 1, 1], [-1, -1, .5], [0, 0, 3]]), np.ones(3)),
            self.grid_pt)
        expected_result = 3
        self.assertEqual(result, expected_result,
                         f"Expected {expected_result} but got {result}!")
