...
class TestWeigherFunctions(TestCase):
    def setUp(self) -> None:
        ...
        self.dict = {"TWS": [1., 1., 3.], "TWA": [1., 2., 8.], "BSP": [1., 3., 3.], "WVHGT": [1., 4., 8.]}
        self.data_bsp = Data().from_dict(self.dict)
        self.data_sog = Data().from_dict({"TWS": [1., 1., 3.], "TWA": [1., 2., 8.], "SOG": [1., 3., 3.], "WVHGT": [1., 4., 8.]})

    def test_hrosailing_standard_scaled_euclidean_norm_default(self):
        """
        Input/Output-Test.
        """
        result = wgh.hrosailing_standard_scaled_euclidean_norm()(self.pts[:, :2])
        expected_result = [.0500771010481109624044336287, 0.0503076952118745368142, 0.151637156266179]
        np.testing.assert_array_almost_equal(result, expected_result, decimal=4,
                                             err_msg=f"Expected {expected_result} but got {result}!")

    def test_hrosailing_standard_scaled_euclidean_norm_custom_2dimensions(self):
        """
        Input/Output-Test.
        """
        result = wgh.hrosailing_standard_scaled_euclidean_norm(["TWS", "sth_else"])(self.pts[:, :2])
        expected_result = [1.0012492197250, 2.000624902374255662, 8.001406126425529714]
        np.testing.assert_array_almost_equal(result, expected_result, decimal=4,
                                             err_msg=f"Expected {expected_result} but got {result}!")

    def test_hrosailing_standard_scaled_euclidean_norm_custom_4dimensions(self):
        """
        Input/Output-Test.
        """
        result = wgh.hrosailing_standard_scaled_euclidean_norm(["TWS", "TWA", "sth_else", "sth_else2"])(self.pts)
    ...
    def test__set_points_from_data_nparr_reduce_False(self):
        ...
        Input/Output-Test.
        """
        result = wgh._set_points_from_data(self.pts[:, :3], self.dimensions, False)
    ...
    def test__set_points_from_data_data_bsp_reduce_no_attrs(self):
        ...
        expected_result = ["TWS", "TWA", "WVHGT"], np.array([[1, 1, 1], [1, 2, 4], [3, 8, 8]])
        np.testing.assert_array_equal(result[0], expected_result[0],
                                      f"Expected {expected_result} but got {result}!")
        np.testing.assert_array_equal(result[1], expected_result[1],
                                      f"Expected {expected_result} but got {result}!")
    ...
    def test__set_points_from_data_Error(self):
        ...
        with self.assertRaises(ValueError):
            wgh._set_points_from_data(Data().from_dict({"not_speed": [1, 2, 3]}), None)
